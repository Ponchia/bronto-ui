 ...
 * persists to localStorage, sets `data-theme` on <html>, and reflects
 * state via `aria-pressed`. A control may set
 * `data-bronto-theme-toggle="dark"` to force a specific theme instead.
 */
export function initThemeToggle({ storageKey = 'bronto-theme', root } = {}) {
  ...
  const reflect = () => {
    ...
    host.querySelectorAll('[data-bronto-theme-toggle]').forEach((el) => {
      el.setAttribute('aria-pressed', String(c === 'dark'));
    });
  };
