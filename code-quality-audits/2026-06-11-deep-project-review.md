...
- **AI-slop helper**: `<local-llm-config>/tools/ai-slop/scan.sh --deep-analysis <repo>` wrote `<local-temp>`. It failed only because the local `osemgrep` shim is broken and Checkov flagged a broad GitHub Actions policy warning.
