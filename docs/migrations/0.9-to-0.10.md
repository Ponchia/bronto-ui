...
`/vue`. Import class names from `@ponchia/ui/classes` and initializers from
`@ponchia/ui/behaviors`. Call each initializer after the DOM root exists and
call its cleanup on unmount. The [integration guide](https://github.com/Ponchia/bronto-ui/blob/main/docs/integration.md) links
