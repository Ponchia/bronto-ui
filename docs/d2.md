...
2. D2 emits each colour twice: as inline `fill="#hex"`/`stroke="#hex"` AND as
   class rules in an embedded `<style>` block (`.fill-N1 { fill: #hex }`).
   **The style rules win over the inline attributes**, so strip the
   hex-bearing rules from the `<style>` first — only then do attribute
